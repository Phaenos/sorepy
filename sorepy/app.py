...
# Saving to file called data.pkl
def save_state(obj):
    try:
        print("Saving...\n")
...
# Setting up automatic backup every hour
async def backup_save(delay = 3600):
    try:
        while True:
...
# NOTE: alredy set up backround backups, currently not needed
# asyncio.run(backup_save()) # Run the automatic backup asynchronously (does not interrupt other code form running)
